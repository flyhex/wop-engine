...
static sfxHandle_t PlayerSettings_MenuKey(int key){
	...
	if(key == K_MWHEELDOWN || key == K_PGDN){
		...
		UI_PlayerInfo_SetInfo(&s_playersettings.playerinfo, LEGS_IDLE, (newweapon == WP_PUNCHY ? TORSO_STAND2 : TORSO_STAND),
...

	{
		...
		for(i = 0; i < ps_playericons.nummodel; ++i){
			...
			if(!Q_stricmpn(modelname, modelskinname, mnLen)){
				...
				s_playersettings.firstmodel = (i + DISPLAYED_MODELS < ps_playericons.nummodel) ? i : ps_playericons.nummodel -
				                                                                                     DISPLAYED_MODELS;
				break;
			}
		}
...
static void PlayerSettings_BuildList(void){
	...
	for(i  = 0; i < numdirs && ps_playericons.nummodel < MAX_UIMODELS; i++, dirptr += dirlen + 1){
		...
		if(!(ps_playericons.modelicons[ps_playericons.nummodel] = trap_R_RegisterShaderNoMip(
				va("models/wop_players/%s/wop_menu", dirptr)))){
			continue;
		}
		ps_playericons.modeliconsB[ps_playericons.nummodel]     = trap_R_RegisterShaderNoMip(
				va("models/wop_players/%s/wop_menuB", dirptr));

		// iterate all skin files in directory
		numfiles = trap_FS_GetFileList(va("models/wop_players/%s", dirptr), "tga", filelist, MAX_MODELFOLDER_FILELIST);
		fileptr  = filelist;
		for(j    = 0;
		    j < numfiles && ps_playericons.lastskinicon[ps_playericons.nummodel] < MAX_SKINS; j++, fileptr += filelen + 1){
			...
			if(!Q_stricmpn(skinname, "icon_", 5)){
				ps_playericons.modelskins[tmpskinnum].icon = trap_R_RegisterShaderNoMip(
						va("models/wop_players/%s/%s", dirptr, skinname));
...
static void PlayerSettings_Draw(void){
	...
	if(s_playersettings.nextGestureTime < uis.realtime){
		UI_PlayerInfo_SetInfo(&s_playersettings.playerinfo, LEGS_IDLE, TORSO_GESTURE, s_playersettings.playerinfo.viewAngles,
		                      vec3_origin, s_playersettings.playerinfo.weapon, qfalse);
	...
	switch(tmpid){
		...
		case ID_SLOGOPREV:
			if(--s_playersettings.slogo_num < 0){
				s_playersettings.slogo_num = uis.spraylogosLoaded - 1;
			}    // close the circle
			break;

		case ID_SLOGONEXT:
			if(++s_playersettings.slogo_num >= uis.spraylogosLoaded){
				s_playersettings.slogo_num = 0;
			}    // close the circle
			break;
	}
}
