...
typedef struct
{
	int type;
	const char *name;
	const char *toolTip; // wop addition: menu tooltips
	int	id;
	int x, y;
	int left;
	int	top;
...
typedef struct
{
	...
	int				height;
	float*			focuscolor;
	qboolean		focuspicinstead; //wop addition by QuarterPounder
} menubitmap_s;
...
// -> wop additions
extern void UI_DrawRectWithThickness(float x, float y, float width, float height, const float *color, int thickness);
