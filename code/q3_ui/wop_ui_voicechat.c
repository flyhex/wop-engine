...
Voice Chat ingame menu - not in use
...
static void UI_VoiceChatMenu_UpdateSelection(int newNum){
	...
	voiceChatMenuInfo.mute.curvalue                                                  = (
			trap_GetVoiceMuteClient(selectedClientID) != 0);

	voiceChatMenuInfo.gain.generic.flags &= ~QMF_GRAYED;
	voiceChatMenuInfo.gain.curvalue =
			trap_GetVoiceGainClient(selectedClientID) * 5;
}
