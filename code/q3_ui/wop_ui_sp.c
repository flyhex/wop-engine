...
WoP SP - discontinued, file not in use

=======================================================================
*/

#include "wop_ui_local.h"
#include "../game/wop_g_spstoryfiles.h"
#include "../game/wop_g_sphandling.h"
...
static void DrawStoryElement(menuStoryElement_t* mse) {
	UI_DrawRectWithThickness(mse->x,mse->y,100,100,colorYellow,1);
	...
	if(shader){
		switch(monitor){
			case 0:
				...
				if(won){
					UI_DrawStringNS(160, 60, "WON!", UI_RIGHT, 12, colorGreen);
				}
				if(!available){
					UI_FillRect(40, 50, 130, 100, colorTBlack66);
				}
				break;
			case 1:
				UI_DrawHandlePic(205, 35, 230, 175, shader);
				if(won){
					UI_DrawStringNS(425, 45, "WON!", UI_RIGHT, 14, colorGreen);
				}
				if(!available){
					UI_FillRect(205, 35, 230, 175, colorTBlack66);
				}
				break;
			case 2:
				UI_DrawHandlePic(470, 50, 130, 100, shader);
				if(won){
					UI_DrawStringNS(590, 60, "WON!", UI_RIGHT, 12, colorGreen);
				}
				if(!available){
					UI_FillRect(470, 50, 130, 100, colorTBlack66);
				}
				break;
		}
	}
