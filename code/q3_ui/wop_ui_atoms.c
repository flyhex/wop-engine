...
void UI_StopMusic(void){
	if(uis.musicbool == qtrue){//nicht stoppen wenn 2(wop_music menu -> exit) oder 0(keine music gestartet)
		trap_S_StopBackgroundTrack();
	}
...
void UI_Init(void){
	...
	if(s_wop_restarted.integer != qtrue){
		...
		//erstmal rausgenommen füür WoP-Engine
...
void UI_DrawRectWithThickness(float x, float y, float width, float height, const float *color, int thickness){
	trap_R_SetColor(color);
...
void UI_Refresh(int realtime){
	...
	if(uis.debug){
		...
		UI_DrawRectWithThickness(1, y, 8 * len + 2, 18, colorBlack, 1);
...
void UI_DrawTextBox (int x, int y, int width, int lines)
{
	...
	UI_DrawRectWithThickness( x + BIGCHAR_WIDTH/2, y + BIGCHAR_HEIGHT/2, ( width + 1 ) * BIGCHAR_WIDTH, ( lines + 1 ) * BIGCHAR_HEIGHT, colorWhite, 1 );
}
