...
static void UI_CallVote_GoEvent(void *ptr, int event){
	...
	if(s_callVoteMenu.numListitems){
		if(Q_stricmp(s_callVoteMenu.votetype.itemnames[s_callVoteMenu.votetype.curvalue], "gametype") == 0){
			...
			tmpStr = va("callvote %s \"%s\"", s_callVoteMenu.votetype.itemnames[s_callVoteMenu.votetype.curvalue], s_callVoteMenu.selItemStr);
		}
	}
	else if(!(s_callVoteMenu.value.generic.flags & (QMF_INACTIVE | QMF_HIDDEN))){
		tmpStr = va("callvote %s \"%s\"", s_callVoteMenu.votetype.itemnames[s_callVoteMenu.votetype.curvalue], s_callVoteMenu.value.field.buffer);
	}
	else{
...
//	gametype = atoi(Info_ValueForKey(info, "g_gametype"));
	...
	s_callVoteMenu.votetype.generic.y        = 148;
	...
	s_callVoteMenu.value.generic.y          = 168;
	...
	s_callVoteMenu.go.generic.y        = 336;
