...
void MusicMenu_Init(void){
	...
	if(NULL == musicInfo.playOrder){
		...
		if(!f){
			...
			Com_Printf(
					MUSIC_MESSAGEPREFIX_S "^1Playlist from disk too large for internal buffer (is %d, max %ld), will be reset!\n",
					i, (sizeof(playlist) - 1));
		...
		if((uis.realtime - musicMenu.switchTime) > 1000){
			if(MUSICSWITCH_OUT == musicMenu.switchState){
				...
				for(t = 0; t < MAX_TRACKS; ++t){
					...
					musicMenu.tracks[t].generic.right = (musicMenu.tracks[t].generic.left +
					                                     strlen(musicInfo.albums[musicMenu.currentAlbum].tracks[t].title) *
					                                     SMALLCHAR_WIDTH);
	...
	if(musicInfo.playOrder != NULL){
		...
		for(playOrder = musicInfo.playOrder, i = 1; playOrder != NULL; playOrder = playOrder->next, i++){
			if(playOrder->album == musicMenu.currentAlbum){
				...
				UI_DrawString((188 + switchOffset), (82 + playOrder->track * 18), va("%2i.", i), (UI_RIGHT | UI_SMALLFONT),
				              colorWhite);
			}
		}
	}
