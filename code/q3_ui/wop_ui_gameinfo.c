...
// @todo: can we abuse this somehow for multiplayer games/detailed scores (ET Pro like)?
/*
===============
UI_LogAwardData
===============
...
void UI_LogAwardData(int award, int data){
	...
	if(award > AWARD_PERFECT){
		trap_Print(va(S_COLOR_RED
		              "Bad award %i in UI_LogAwardData\n", award));
		return;
	}
...
void UI_LoadLogoForMenu(const char *spraylogoName){
	...
	uis.spraylogoShaders[uis.spraylogosLoaded] = trap_R_RegisterShaderNoMip(va(SPRAYLOGO_PATH
	                                                                           "/%s", spraylogoName));
