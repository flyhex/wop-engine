	...
	for( column = 0; column < l->columns; column++ ) {
		...
		for( i = base; i < base + l->height; i++) {
			...
			if(sLen > l->width){
				...
				if(i == l->curvalue){
					Q_strncpyz(buff,
					           l->itemnames[i] + (int)((sLen - j + 1) * (0.5f * sin(uis.realtime * 0.02f / (sLen - j)) + 0.5f)),
					           j + 1);
...
void UI_DrawToolTip(menucommon_s *focusItem){
	...
	UI_DrawRectWithThickness(boxX, boxY, boxWidth, boxHeight, colorYellow, 2);
...
void Menu_Draw( menuframework_s *menu )
{
		...
		if( uis.debug ) {
			...
			if( !( itemptr->flags & QMF_INACTIVE ) ) {
				...
				if (itemptr->flags & QMF_HASMOUSEFOCUS) {
					UI_DrawRectWithThickness(x, y, w, h, colorYellow, 1 );
				}
				else {
					UI_DrawRectWithThickness(x, y, w, h, colorWhite, 1 );
				}
			}
		}
#endif
	...
	if(uis.dropdownlist){
		...
		for(i = 0; i < uis.dropdownlist->numitems; i++){
			...
			UI_DrawStringNS(uis.dropdownxywh[0], uis.dropdownxywh[1] + i * SMALLCHAR_HEIGHT, uis.dropdownlist->itemnames[i],
			                UI_LEFT | UI_SMALLFONT, 16, uis.dropdownlist->dropdown_text);
		}

		UI_DrawRectWithThickness(dropdownBG[0], dropdownBG[1], dropdownBG[2], dropdownBG[3], uis.dropdownlist->dropdown_border, 2);
	}
...
void Menu_Cache( void ){
	...
	menu_in_sound     = trap_S_RegisterSound("sounds/menu/mouse_over", qfalse);
	menu_out_sound    = trap_S_RegisterSound("sounds/menu/menu_error", qfalse);
