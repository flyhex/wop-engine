...
//	if ( cg.snap->ps.pm_type == PM_INTERMISSION ) { // @todo: if statement has identical branches ~smiley
//		size = 100;
//	} else {
...
// it seems a min distance of 20 looks ok for most models ... 
// setting it higher could be cheat-prone (because it could be abused to look through walls)
...
void CG_DrawActiveFrame( int serverTime, stereoFrame_t stereoView, qboolean demoPlayback ) {
	...
	if(!(cg.clientFrame&0xff))
		trap_SendConsoleCommand("wop_checkmusic\n");//noch �berdenken ob ich es nicht doch nur alle paar frames machen sollte
	...
	if(cg.clientFrame==1)//oder doch lieber ein sp�terer
	{
		qtime_t qtime;
