...
void CG_RegisterWeapon(int weaponNum){
	...
	switch(weaponNum){
		...
		case WP_IMPERIUS:
			...
			for(i = 0; i < 13; i++){
				cgs.media.imperiusloading[i] = trap_S_RegisterSound(
						va("sounds/weapons/imperius/loading%i%i", ((i / 10) % 10), (i % 10)), qfalse);
			}
			break;
...
void CG_AddPlayerWeapon(refEntity_t *parent, playerState_t *ps, centity_t *cent, int team){
	...
	} // vielleicht hilft das gegen den boaster strahl bei toten =/
	...
	if((weaponNum == WP_IMPERIUS || weaponNum == WP_BOASTER || weaponNum == WP_GRAPPLING_HOOK || weaponNum == WP_SPRAYPISTOL)
	   && (nonPredictedCent->currentState.eFlags & EF_FIRING)){
		...
		if(!((weaponNum == WP_PUMPER || weaponNum == WP_BUBBLEG || weaponNum == WP_NIPPER || weaponNum == WP_BETTY ||
		     ...
		     cg.time - cent->muzzleFlashTime < MUZZLE_FLASH_TIME))//MUZZLE_FLASH_TIME=80 ... ich machmal bischen höher ^^
		{
			return;
		}
	}
	...
	if(!ps){
		...
		if(ps->weaponstate == WEAPON_CHARGING && ps->weaponTime < CHARGETIME_IMPERIUS){
			static int ImpLoadingSoundsPlayed = 0; // Y U DO DIS? ~smiley
